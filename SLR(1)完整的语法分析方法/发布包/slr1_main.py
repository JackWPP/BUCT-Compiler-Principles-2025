# -*- coding: utf-8 -*-
"""
编译原理作业 - SLR(1)完整的语法分析方法
实现上下文无关文法的识别和拓广，LR(0)识别活前缀的状态机，SLR(1)分析表，LR分析过程
...
class SLR1ParsingTable:
    """SLR(1)分析表类"""

    def __init__(self, grammar: Grammar, automaton: LRAutomaton):
        self.grammar = grammar
    ...
    def _add_shift_actions(self, state_index: int, state: FrozenSet[LRItem]):
        """添加移进动作"""
        for item in state:
            if not item.is_complete():
                next_sym = item.next_symbol()
    ...
    def _add_reduce_actions(self, state_index: int, state: FrozenSet[LRItem]):
        """添加归约动作"""
        for item in state:
                ...
                for terminal in follow_set:
                    ...
                    if action_key in self.action_table:
                        ...
                        if existing_action.action_type == ActionType.SHIFT:
                            self.conflicts.append(
                                f"状态{state_index}，符号'{terminal}': 移进-归约冲突"
                            )
                        elif existing_action.action_type == ActionType.REDUCE:
                            self.conflicts.append(
                                f"状态{state_index}，符号'{terminal}': 归约-归约冲突"
                            )
                    else:
                        self.action_table[action_key] = new_action
        ...
        for state_index, state in enumerate(self.automaton.states):
            for item in state:
                if (item.is_complete() and
                    ...
                    item.production.left == self.grammar.start_symbol):
...
class SLR1Parser:
    ...
    def __init__(self, grammar: Grammar):
        ...
        self.parsing_table = SLR1ParsingTable(self.grammar, self.automaton)
...
class SLR1ParserGUI:
    ...
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("SLR(1)完整的语法分析方法 - GUI版本")
        self.root.geometry("1600x1000")

        # 初始化组件
        self.current_grammar: Optional[Grammar] = None
        self.current_parser: Optional[SLR1Parser] = None
...
F -> ( E ) | id"""
    ...
    def create_parsing_table_tab(self, parent):
        """创建SLR(1)分析表标签页"""
        frame = ttk.Frame(parent)
        parent.add(frame, text="SLR(1)分析表")
        ...
        ttk.Label(table_frame, text="SLR(1)分析表:", font=("Arial", 12, "bold")).pack(anchor=tk.W)
    ...
    def create_visualization_tab(self, parent):
        ...
        ttk.Button(control_frame, text="生成LR(0)自动机", command=self.generate_automaton_visualization).pack(side=tk.LEFT, padx=(0, 10))
...
1. 文法输入
   ...
   • 设置开始符号
   • 点击"解析文法"按钮解析文法
   • 点击"构建分析器"按钮构建SLR(1)分析器

2. SLR(1)分析表
   • 在"SLR(1)分析表"标签页中查看生成的分析表
...
3. 语法分析过程
   ...
   • 点击"开始分析"执行SLR(1)语法分析
   • 查看详细的分析步骤，包括栈状态、输入缓冲区和执行的动作
   • 查看分析结果（成功/失败）和错误信息

4. 自动机可视化
   • 在"自动机可视化"标签页中查看LR(0)自动机的状态转换图
   • 点击"生成LR(0)自动机"生成可视化图形
   • 选择特定状态查看状态详情
   • 可保存图片
...
"""
    ...
    def build_parser(self):
        ...
        try:
            # 创建SLR(1)分析器
            self.current_parser = SLR1Parser(self.current_grammar)
            ...
            if self.current_parser.parsing_table.has_conflicts():
                ...
                messagebox.showinfo("成功", "SLR(1)分析器构建成功，无冲突")
    ...
    def check_conflicts(self):
        ...
        if conflicts:
            ...
            self.conflict_info.insert(tk.END, "无冲突，文法是SLR(1)文法")
